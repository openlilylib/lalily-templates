...
(define-module (lalily-templates scheme store))
                                    ...
                                    (let ((oplist (tree-collect templates piece (stack-create)))
                                      ...
                                      (for-each (lambda (p)
                                                  (let* ((o (if (pair? p)(cdr p) '()))
                                                         (ohead (ly:assoc-get '_header opts '() #f))
                                                         (nhead (ly:assoc-get '_header o '() #f))
                                                         (head (assoc-set-all! ohead nhead)))
                                                    (set! opts (assoc-set-all! opts o))
                                                    (if (> (length head))(assoc-set! opts '_header head))
                                                    )) (reverse oplist))
...
(define-public (set-default-header piece field value)
  (let* ((tmpl (get-default-template piece))
         (opts (get-default-options piece))
         (header (ly:assoc-get '_header opts '() #f)))
    (set! header (assoc-set! header field value))
    (set! opts (assoc-set! opts '_header header))
